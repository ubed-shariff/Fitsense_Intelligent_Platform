...
async def get_workout_plan(current_user: schemas.UserInDB = Depends(get_current_user)):
    user_profile = {
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(f"{ML_SERVICE_URL}/predict/workout", json=user_profile)
